import { MARKET_LISTING_SOURCES } from "@/types/market";
import type {
...
export function normalizeSource(value: unknown): MarketListingSource {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
  if (!normalized) throw new Error("Market ingestion: source is required.");
  if (!MARKET_LISTING_SOURCES.includes(normalized as MarketListingSource)) {
    throw new Error(`Market ingestion: unsupported source "${normalized}".`);
  }
  return normalized as MarketListingSource;
}
...
export function buildRawListingPayload(input: MarketListingRawInput) {
  const source = normalizeSource(input.source);
  const sourceUrl = normalizeOptionalString(input.sourceUrl);

  if (!sourceUrl) {
    throw new Error("Market ingestion: source_url is required.");
  }
