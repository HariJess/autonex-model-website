...
import type {
  ...
  MarketListingRawInput,
  MarketListingRawRecord,
} from "@/types/market";
import { buildRawListingPayload } from "@/lib/market/rawListingValidation";

type MarketSupabaseClient = SupabaseClient;
type RawListingUpsertPayload = ReturnType<typeof buildRawListingPayload>;

function pickIncomingString(
...
function pickLatestTimestamp(
  ...
  if (incomingTs == null) return new Date(existingTs as number).toISOString();
...
function mergeRawPayloadNonDestructive(
  existing: MarketListingRawRecord | null,
  incoming: RawListingUpsertPayload,
): RawListingUpsertPayload {
...
async function getRawBySourceAndUrl(
  client: MarketSupabaseClient,
  source: string,
  sourceUrl: string,
): Promise<MarketListingRawRecord | null> {
  ...
  return (data as MarketListingRawRecord | null) ?? null;
}
...
export function createMarketAdminClientFromEnv(): MarketSupabaseClient {
  ...
  if (!url || !serviceRoleKey) {
    throw new Error(
      "Market ingestion requires SUPABASE_URL (or VITE_SUPABASE_URL) and SUPABASE_SERVICE_ROLE_KEY.",
    );
  }
...
export async function upsertMarketRawListing(
  client: MarketSupabaseClient,
  input: MarketListingRawInput,
): Promise<MarketListingRawRecord> {
  ...
  return data as MarketListingRawRecord;
}

export async function insertManyMarketRawListings(
  client: MarketSupabaseClient,
  inputs: MarketListingRawInput[],
  ...
  const records: MarketListingRawRecord[] = [];
  const errors: string[] = [];
