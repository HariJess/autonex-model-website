  ...
  return (
    <>
      ...
      <article className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="text-muted-foreground font-sans mb-8 text-sm flex flex-wrap items-center gap-2">
        ...
        <div className="space-y-8 font-sans">
          <p className="text-lg leading-relaxed text-foreground font-medium">{post.intro}</p>
          {post.sections.map((section) => (
            <section key={section.heading} className="space-y-4">
              <h2 className="font-serif text-2xl font-bold text-foreground">{section.heading}</h2>
              {section.paragraphs?.map((paragraph, idx) => (
                <p key={idx} className="text-muted-foreground leading-7">{paragraph}</p>
              ))}
