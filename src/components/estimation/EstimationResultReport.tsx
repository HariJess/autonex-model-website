import { ArrowRight, CarFront, ChevronRight, Info, Target, TrendingUp, Shield } from "lucide-react";
import { Link } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { formatAriary } from "@/lib/estimation/constants";
import type { EstimationPresentation } from "@/lib/estimation/presentation";
...
export default function EstimationResultReport({
  ...
  // PROMPT 10B — V2 detection : audit field présent => engine V2
  const isV2 = Boolean(v2.audit);
  const { data: freshness } = useDataFreshness();

  // PROMPT 10D — Sépare comparables AutoNex (cliquables) vs marché public (mkt:, non-cliquables)
  const mktComparables = comparables.filter((c) => c.listingId.startsWith("mkt:"));
  const anxComparables = comparables.filter((c) => !c.listingId.startsWith("mkt:"));

  return (
    <section className="space-y-4 md:space-y-5" aria-label={t("estimation.report.sectionAria", "Rapport d'estimation AutoNex")}>
          ...
          <div className="grid gap-4 md:grid-cols-[1.6fr_1fr] md:items-end">
            <div className="space-y-3">
              ...
              <div className="rounded-2xl border border-background/30 bg-background/12 px-4 py-3">
                <p className="font-sans text-[11px] uppercase tracking-wide text-background/60">{t("estimation.report.valuationRange", "Fourchette de valorisation")}</p>
            ...
            <div className="rounded-2xl border border-background/30 bg-background/[0.14] p-4 backdrop-blur-sm shadow-inner">
              ...
              <div className="mt-2 flex items-end gap-2">
                {presentation.showExactConfidence ? (
                  <>
                    <p className="font-sans text-4xl leading-none tabular-nums">{presentation.confidenceDisplayValue}</p>
                    <p className="pb-0.5 font-sans text-sm text-background/68">/100 · {presentation.summaryLevel}</p>
                  </>
                ) : (
                  <p className="font-sans text-xl leading-none text-background/90">{t("estimation.report.cautiousDisplay", "Affichage prudent")}</p>
                )}
              </div>
      ...
      {/* PROMPT 10D — 2 cards Argus (Reprise pro / Concession) — "Entre particuliers" supprimé */}
      <ArgusValuesCard
        values={{
      ...
      {/* PROMPT 10D — Repères vendeur (Conseillé / Rapide) compactés en bloc discret */}
      <div className="rounded-xl border border-border/60 bg-muted/30 p-4">
        <p className="font-sans text-[11px] uppercase tracking-wide text-muted-foreground mb-3">
          {t("estimation.report.sellerReferencesTitle", "Repères de positionnement")}
        </p>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <p className="font-sans text-sm text-muted-foreground">{t("estimation.report.recommendedListingPrice", "Prix conseillé d'annonce")}</p>
            <p className="mt-0.5 font-sans text-lg font-semibold text-foreground tabular-nums">{formatAriary(values.recommendedListingPrice)}</p>
          </div>
          <div>
            <p className="font-sans text-sm text-muted-foreground">{t("estimation.report.quickSalePrice", "Prix de vente rapide")}</p>
            <p className="mt-0.5 font-sans text-lg font-semibold text-foreground tabular-nums">{formatAriary(values.quickSalePrice)}</p>
          </div>
        </div>
      </div>
      ...
      {/* PROMPT 10D — Logique comparables intelligente :
          - Cas A : que des mkt: → bandeau info discret (pas de cards cliquables 404)
          - Cas B : ≥1 anx: → cards cliquables anx + ligne "+N marché public" si mix
          - Cas C : aucun comp → rien (pas de section) */}
      {anxComparables.length > 0 && (
        <Card className="rounded-2xl border border-border/60 bg-card/95 shadow-sm transition-all duration-300 ease-out hover:shadow-md">
          ...
          <CardContent>
            ...
            {mktComparables.length > 0 && (
              <p className="mt-3 font-sans text-xs text-muted-foreground" data-testid="comparables-mkt-extra-line">
                {t("estimation.report.publicMarketExtra", "+ {{n}} autres références du marché public utilisées dans le calcul.", { n: mktComparables.length })}
              </p>
            )}
          </CardContent>
        </Card>
      ...
      {anxComparables.length === 0 && mktComparables.length > 0 && (
        <div
          className="rounded-xl border border-border/60 bg-muted/30 px-4 py-3"
          data-testid="comparables-mkt-only-banner"
        >
          <p className="font-sans text-sm text-muted-foreground flex items-center gap-2">
            <Info className="h-4 w-4 shrink-0" aria-hidden />
            {t("estimation.report.publicMarketBanner", "Estimation calculée à partir de {{n}} références du marché public.", { n: mktComparables.length })}
          </p>
        </div>
      )}
      ...
      <Card className="rounded-3xl border border-primary/25 bg-gradient-to-br from-primary/[0.11] via-primary/[0.05] to-background shadow-md">
        <CardContent className="p-5 md:p-7">
          ...
          <div className="mt-4 grid grid-cols-1 gap-2.5 md:mt-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-border/55 bg-background/65 px-3 py-2">
