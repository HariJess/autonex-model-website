    ...
    return (
      <Card className="rounded-2xl">
        <CardContent className="p-6 flex items-center gap-2 text-sm text-muted-foreground font-sans">
          <Loader2 className="h-4 w-4 animate-spin" /> {t("agencies.myAgency.loading", "Chargement de votre agence...")}
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className="rounded-2xl">
        <CardContent className="p-6 text-sm text-destructive font-sans">
          {error instanceof Error ? error.message : t("agencies.myAgency.error", "Erreur")}
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className="rounded-2xl">
          ...
          {agency.status === "approved" ? (
            <Button variant="outline" size="sm" asChild className="font-sans">
              <a href={`/concessionnaires/${agency.slug}`} target="_blank" rel="noreferrer">
                <ExternalLink className="h-4 w-4 mr-1" /> {t("agencies.myAgency.viewPublicProfile", "Voir ma fiche publique")}
              </a>
            </Button>
          ) : null}
        </div>
        <CardDescription className="font-sans pt-2">
          {t("agencies.myAgency.editPublicInfoIntro", "Modifiez les informations visibles publiquement.")}{" "}
          {t("agencies.myAgency.legalFieldsHint", "Les champs légaux (nom, NIF, STAT, RC, adresse) restent gérés par l'admin.")}
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-5 font-sans">
        ...
        <section className="space-y-3">
          <h3 className="font-sans text-sm font-semibold">{t("agencies.myAgency.visualIdentityTitle", "Identité visuelle")}</h3>
          <div>
            <Label htmlFor="my-logo">{t("agencies.myAgency.logoUrl", "Logo URL")}</Label>
            <Input id="my-logo" type="url" value={f.logo_url} onChange={(e) => set("logo_url", e.target.value)} />
          </div>
          <div>
            <Label htmlFor="my-cover">{t("agencies.myAgency.coverImageUrl", "Image de couverture URL")}</Label>
            <Input id="my-cover" type="url" value={f.cover_image_url} onChange={(e) => set("cover_image_url", e.target.value)} />
          </div>
        </section>

        <section className="space-y-3">
          <h3 className="font-sans text-sm font-semibold">{t("agencies.myAgency.aboutTitle", "Présentation")}</h3>
          <div>
            <Label htmlFor="my-bio">{t("agencies.myAgency.shortBio", "Bio (courte)")}</Label>
            <Textarea id="my-bio" rows={2} value={f.bio} onChange={(e) => set("bio", e.target.value)} />
          </div>
          <div>
            <Label htmlFor="my-desc">{t("agencies.myAgency.detailedDescription", "Description détaillée")}</Label>
            <Textarea id="my-desc" rows={5} value={f.description_long} onChange={(e) => set("description_long", e.target.value)} />
          </div>
        </section>

        <section className="space-y-3">
          <h3 className="font-sans text-sm font-semibold">{t("agencies.myAgency.contactTitle", "Contact")}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              <Label htmlFor="my-email">{t("auth.email", "Email")}</Label>
              <Input id="my-email" type="email" value={f.email} onChange={(e) => set("email", e.target.value)} />
            </div>
            <div>
              <Label htmlFor="my-phone">{t("auth.phone", "Téléphone")}</Label>
              <Input id="my-phone" value={f.phone} onChange={(e) => set("phone", e.target.value)} />
            </div>
            <div>
              <Label htmlFor="my-wa">{t("agencies.myAgency.whatsapp", "WhatsApp")}</Label>
              <Input id="my-wa" value={f.whatsapp_phone} onChange={(e) => set("whatsapp_phone", e.target.value)} />
            </div>
            <div>
              <Label htmlFor="my-web">{t("agencies.myAgency.website", "Site web")}</Label>
        ...
        <section className="space-y-3">
          <h3 className="font-sans text-sm font-semibold">{t("agencies.myAgency.openingHoursTitle", "Horaires")}</h3>
        ...
        <section className="space-y-3">
          <h3 className="font-sans text-sm font-semibold">{t("agencies.myAgency.socialMediaTitle", "Réseaux sociaux")}</h3>
