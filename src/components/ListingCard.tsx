  ...
  return (
    <div className="group rounded-2xl overflow-hidden bg-card border border-border shadow-sm hover:shadow-lg transition-all duration-300">
      <div className="relative aspect-[4/3] overflow-hidden">
        <img src={listing.images[imgIndex]} alt={listing.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        {listing.badge && (
        ...
        {listing.images.length > 1 && (
          <>
            <button onClick={(e) => { e.preventDefault(); setImgIndex((i) => (i > 0 ? i - 1 : listing.images.length - 1)); }} className="absolute left-2 top-1/2 -translate-y-1/2 p-1 rounded-full bg-card/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={(e) => { e.preventDefault(); setImgIndex((i) => (i < listing.images.length - 1 ? i + 1 : 0)); }} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full bg-card/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <Link to={`/annonce/${listing.id}`} className="block p-4 space-y-2">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground font-sans">
          {listing.rooms > 0 && <span className="flex items-center gap-1"><Bed className="h-3.5 w-3.5" />{listing.rooms}</span>}
