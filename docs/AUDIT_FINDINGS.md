...
- 🔴 **BLOCKER PRÉ-P.3 — Supabase JWT algorithm mismatch (ES256 vs HS256)** : Supabase émet des JWT en ES256 (migration récente côté projet) mais les Edge Functions qui ont `verify_jwt=ON` valident encore avec le legacy HS256 secret. Résultat : tout appel user-authenticated à `vpi-check-status` et `vpi-initiate-payment` retourne `401 UNAUTHORIZED_UNSUPPORTED_TOKEN_ALGORITHM` / "Unsupported JWT algorithm ES256". Confirmé Mission P.2 Étape 8.B (smoke test login → <test user>) : TESTS 1-6 PASS, TEST 7 BLOCKED, TEST 8 E2E impossible à démarrer via flow user-auth.

  **Hypothèse "send-contact-email tourne en prod donc blocker pas réel" invalidée** : check direct (probes A-C sur send-contact-email sans/avec JWT ES256) → cette fonction a `verify_jwt=OFF` (Dashboard override), donc son fonctionnement n'apporte aucune info sur le comportement du gateway ES256. À contrario, `vpi-initiate-payment` a `verify_jwt=ON` (probe D → 401 NO_AUTH_HEADER), donc son 401 ES256 est bien un blocker prod pour les users qui paient via frontend.
 Actions possibles :
  - a) Dans le code Edge, remplacer la vérif legacy par une vérif JWKS (`${SUPABASE_URL}/auth/v1/keys`) qui supporte ES256 — recommandé, robuste, future-proof.
  - b) Forcer Supabase à émettre HS256 (downgrade projet) — non recommandé, régression de sécurité.
  - c) Custom JWT verify dans le code de chaque fonction au lieu de se reposer sur "Verify JWT with legacy secret" — option intermédiaire.

  À traiter **impérativement avant Mission P.3** (frontend VPI) sinon tout le checkout côté user sera cassé en prod. Le backend VPI (webhook HMAC, RPC approve, idempotence credits_granted_at) reste validable via service_role mais pas via le flow E2E authentifié tant que ce blocker tient.
