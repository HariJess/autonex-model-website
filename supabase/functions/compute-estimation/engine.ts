// Supabase Edge Function — compute-estimation / engine.ts
// -----------------------------------------------------------------------------
// Portage 1:1 du moteur client `src/lib/estimation/engine.ts` vers une cible
// Edge Function Deno. Le module est volontairement PUR (pas d'import Deno-only,
// pas d'import global de client supabase) pour rester importable depuis Node
// (Vitest) afin que les tests de parité legacy vs v2 puissent s'exécuter sur
// le même runtime de test.
//
// Le client supabase est INJECTÉ via un paramètre — `index.ts` le construit
// avec SUPABASE_SERVICE_ROLE_KEY, les tests Node fournissent un mock qui
// reproduit exactement les mêmes datasets que la voie legacy.
//
// ⚠️ Toute évolution de logique doit être faite ICI ET dans le moteur legacy
// jusqu'à ce que le rollout v2 soit à 100% et que le legacy soit dépréciable.
// Garder la parité comportementale stricte est l'invariant non-négociable.
// -----------------------------------------------------------------------------
...
// PROMPT 10E — Couche 2 (proximity) + Couche 4 (sanity bounds) — parité TS
import {
  findProxyModels,
  proximityFactorMid,
  PROXIMITY_SIMILARITY_CEILING,
} from "./modelProximity.ts";
import { applySanityCheck } from "./sanityBounds.ts";

/**
 * Interface minimale couvrant les appels supabase-js utilisés par le moteur.
 * Permet d'éviter une dépendance forte à `@supabase/supabase-js` côté tests
 * Node tout en restant compatible avec le client réel (Deno + Node).
 */
export interface MinimalQueryBuilder {
  select: (cols: string) => MinimalQueryBuilder;
  eq: (col: string, val: unknown) => MinimalQueryBuilder;
  ilike: (col: string, pattern: string) => MinimalQueryBuilder;
  ...
  order: (col: string, opts?: { ascending?: boolean }) => MinimalQueryBuilder;
...
type ComparableListingRow = {
  ...
  make: string | null;
  model: string | null;
  /** PROMPT 10E — Couche 2 : type de proximité (undefined si exact). */
...
function computeSimilarityScore(input: EstimationInput, candidate: QualityAcceptedCandidate): number {
  ...
  const makeMatch = toNormalized(row.make ?? "") === toNormalized(input.makeName);
  const modelMatch = toNormalized(row.model ?? "") === toNormalized(input.modelName);
...
async function fetchComparableRows(
  ...
  let query = client
    .from("listings")
    .select(
      "id,title,price_mga,year,mileage_km,ville,region,body_style,fuel,transmission_gearbox,description,created_at,updated_at,status,make,model",
    )
    .eq("status", "active")
  ...
  const { data, error } = (await query) as { data: ComparableListingRow[] | null; error: unknown };
  if (error) {
...
async function fetchComparables(
  ...
  if (proxyConfig) {
    ...
    for (const proxyKey of proxyConfig.proxyModels) {
      ...
      const [proxyMkt, proxyStrict] = await Promise.all([
        fetchMarketCleanRows(client, proxyInput, { yearWindow: 5, limit: 80 }),
        fetchComparableRows(client, proxyInput, { yearWindow: 4, strictAttributes: false, limit: 80 }),
      ]);
      ...
      proxyMkt.forEach((row) => {
        if (!allById.has(row.id)) {
          allById.set(row.id, { row: tag(row), source: "strict" });
        }
      });
  ...
  // PROMPT 10E — Couche 2 : applique aussi proximityFactor si row segment_proche
  ...
  // PROMPT 10E — Couche 2 : plafond similarité PROXIMITY_SIMILARITY_CEILING (75) sur les proxy
...
/**
 * Entrée principale de la fonction Edge.
 * Reçoit un client supabase (service-role côté Deno, mock côté tests Node).
 *
 * Cette fonction DOIT rester byte-for-byte équivalente à
...
export async function computeVehicleEstimationV2(
  client: MinimalSupabaseClient,
  input: EstimationInput,
): Promise<EstimationOutputV2> {
  const normalized = normalizeInput(input);
  const profile = await findReferenceProfile(client, normalized);
