-- Prefer commercial_contact_name for profiles.full_name when role is agence (avoids wrong full_name in metadata).
...
BEGIN
  ...
  ELSE
    profile_full_name := COALESCE(NULLIF(trim(meta->>'full_name'), ''), '');
  END IF;
